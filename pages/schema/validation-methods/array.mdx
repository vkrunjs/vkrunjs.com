...
const validateA = schema.validate(["hello", "world"]) // true
const validateB = schema.validate(["hello"])         // false
...
const validateA = schema.validate(["hello", "world", "test"]) // true
const validateB = schema.validate(["hello", "world", "test", "extra"]) // false
...
const validateA = schema.validate(["hello", "world"]) const validateB = schema.validate([1, 2])

console.log(validateA) // true console.log(validateB) // false
```
...
const validateA = schema.validate([1, 2, 3]) const validateB = schema.validate([1, "2"])
...
const validateA = schema.validate([true, false]) const validateB = schema.validate([true, "false"])
...
```ts
const schema = schema().array().object({ id: schema().string().UUID(), fullName: schema().string().minWord(2) })

const validateA = schema.validate([ { id: "3ef7c105-c4ea-444d-bf47-e2e1a49ea613", fullName: "Full Name" } ]) const validateB = schema.validate([ { id: "3ef7c105-c4ea-444d-bf47-e2e1a49ea613" } ])
...
const validateA = schema.validate([null]) const validateB = schema.validate([undefined])

console.log(validateA) // true console.log(validateB) // true
```
...
const validateA = schema.validate(undefined) // Allowed because it's not required const validateB = schema.validate(["value"]) // Valid array
...
const validateA = schema.validate([true]) // Matches schema().boolean() const validateB = schema.validate(["hello"]) // Matches schema().string() const validateC = schema.validate([1]) // No match

console.log(validateA) // true console.log(validateB) // true console.log(validateC) // false
```
...
const validateA = schema.validate([1]) // No match const validateB = schema.validate([true]) // Match schema().boolean() const validateC = schema.validate(["hello"]) // Match schema().string()

console.log(validateA) // true console.log(validateB) // false console.log(validateC) // false
```
